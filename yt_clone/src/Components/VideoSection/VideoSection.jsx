...
const VideoSection = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await fetch(`${API_URL}&q=programming `)
  ...
  if (videos.length === 0) {
    return <div className={styles.noVideos}>No videos found</div>
  }
      ...
      {videos.map((video) => (
        <div key={video.id.videoId} className={styles.videoCard}>
          ...
          <div className={styles.videoInfo}>
            ...
            <p className={styles.videoMeta}>{new Date(video.snippet.publishTime).toLocaleDateString()}</p>
          </div>
        </div>
      ))}
    </div>
